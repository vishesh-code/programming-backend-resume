...
// GET ALL PROBLEMS (Filtered by My Problems, Public, or Shared)
router.get('/', verify, async (req, res) => {
  try {
    const tab = req.query.tab || 'my_problems';
    let queryFilter = {};
    ...
    if (tab === 'public') {
      ...
      queryFilter = { sharedWith: currentUser.email };
    } else {
      queryFilter = { user: req.user._id }; // Default: My Problems
    }
    ...
    let query = Problem.find(queryFilter)
      .populate('category', 'name slug') 
      .populate('tags', 'name slug')     
      .sort({ createdAt: -1 });
...
router.post('/', verify, async (req, res) => {
  const { 
    question, description, solutions, difficulty, 
    category, tags, time_complexity, space_complexity,
    visibility // 🔥 NEW
  } = req.body;
  ...
  const problem = new Problem({
    ...
    time_complexity,
    space_complexity,
    visibility: visibility || "Private", // 🔥 NEW
    solved: false
  });
...
router.post('/:id/share', verify, async (req, res) => {
  try {
    ...
    // Only allow the owner to share it
    const problem = await Problem.findOne({ _id: req.params.id, user: req.user._id });
    if (!problem) return res.status(404).json({ message: "Problem not found or unauthorized" });

    // Add email if not already shared
    if (!problem.sharedWith.includes(email.toLowerCase())) {
      problem.sharedWith.push(email.toLowerCase());
      await problem.save();
    }

    res.json({ message: `Successfully shared with ${email}` });
  } catch (error) {
    res.status(500).json({ message: "Error sharing problem", error: error.message });
  }
});
